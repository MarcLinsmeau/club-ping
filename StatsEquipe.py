...
def execution_app(conn):
    """Conteneur : Synthèse hebdomadaire avec index (Semaine, Equipe2), colonnes Joueurs."""
    ...
    if not (st.session_state.annee_choisie and st.session_state.club_choisi and st.session_state.division_choisie):
        ...
        if df_res.empty:
            ...
            # 1. Calcul agrégé (Semaine, Equipe2 en index)
            ...
            # 2. Tri joueurs alphabétique
            joueurs = sorted(df_g.index.get_level_values("Joueur1").unique())
            
            # 3. Construction + conversion en Int64 (pour éviter les virgules)
            df_list = []
            ...
            # 5. Affichage avec style (quadrillage accentué)
            ...
            # Hauteur dynamique pour essayer d'éliminer le scroll vertical
