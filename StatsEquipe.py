...
def execution_app(conn):
    """Conteneur : Synthèse hebdomadaire avec ajustement dynamique pour supprimer le scroll."""
    ...
    if not (st.session_state.annee_choisie and st.session_state.club_choisi and st.session_state.division_choisie):
        ...
        if df_res.empty:
            ...
            # 4. Affichage optimisé sans barres de scroll
            ...
            # Application de la technique de hauteur dynamique
            # On calcule la hauteur nécessaire en multipliant par le nombre de lignes + marge
            hauteur_totale = (len(df_pivot) + 2) * 35 
