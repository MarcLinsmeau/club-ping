...
def execution_app(conn):
    """Conteneur : Semaine (index), Indicateurs (sous-index), Joueurs (colonnes)."""
    ...
    if not st.session_state.annee_choisie or not st.session_state.clubs_choisis or not st.session_state.divisions_choisies:
        ...
        if df_res.empty:
            ...
            # 2. Transposition : on "stack" les indicateurs en ligne, puis "unstack" le joueur en colonnes
            df_pivot = df_g.stack().unstack(level="Joueur1")
            
            # 3. Tri des semaines (Index)
            df_pivot = df_pivot.sort_index(key=lambda x: x.map(utils.parse_semaine))

            st.subheader(f"📋 Synthèse par Semaine & Indicateurs ({len(df_res)} match(s))")
            
            # 4. Affichage HTML avec alignement forcé
            html_table = (
                df_pivot.style.format("{:.0f}") # Format par défaut
                .format(lambda x: f"{x:.1f}%", subset=pd.IndexSlice[:, pd.IndexSlice[:, "% Victoire"]]) # Format % spécifique
                .set_table_styles([
